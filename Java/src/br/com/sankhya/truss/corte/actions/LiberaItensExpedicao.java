...
import com.hazelcast.org.apache.calcite.adapter.java.Array;
...
public class LiberaItensExpedicao implements AcaoRotinaJava {
	...
	public void doAction(ContextoAcao ctx) throws Exception {
		...
		try {
			...
			if(!validaStatus(linhas)) {
				ctx.mostraErro("Npossusar esta rotina. O pedido deve ter o faturamento ou atendimento liberado.");
				return;
			}
			...
			if(!"S".equals(liberaIntegral) && qtdLiberar.equals(BigDecimal.ZERO)) {
				ctx.mostraErro("Se nliberar o pedido integralmente, um valor de itens liberados deve ser informado.");
			}
			...
			for(Registro linha : linhas) {
				...
				if(qtdMinVenda.compareTo(BigDecimal.ZERO) <= 0) {
					ctx.mostraErro("Produto sem quantidade mde venda cadastrado.");
				}

				if(!isMultiplo(qtdLiberar, qtdMinVenda)) {
					ctx.mostraErro("Indique uma quantidade mda qtd. mde venda do produto: " + qtdMinVenda);
				}
				...
				if(qtdLiberar.compareTo(disponivel) > 0) {
					ctx.mostraErro("Nexiste quantidade disponsuficiente para liberar a quantidade solicitada. \n Quantidade Dispon" + disponivel);
				}
	...
	private void liberaIntegral(BigDecimal nunota) throws Exception {
		...
		for(DynamicVO iteVO : iteDAO.find("NUNOTA = ? AND QTDCONFERIDA > 0", nunota)) {
			...
			if(continua.equals("S")) {
				...
				count++;
				mensagemErro = "Itens Liberados. Alguns itens npuderam ser liberados por falta de estoque. Verifique o campo Qtd. Corte na aba de itens.";
			}
		}
	...
	public static boolean isMultiplo(BigDecimal valor, BigDecimal multiplo) {
		...
		// Um nmse o resto da divisfor zero
